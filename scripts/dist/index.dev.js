...
$(document).ready(function () {
  ...
  function showTop() {
    ...
    $(response.top).each(function (index) {
      ...
      sb += '<div class="item">' + '<div class="rank">' + response.top[index].rank + '</div>' + '<div class="title">' + response.top[index].title + '</div>' + '<div class="score ' + getScoreColor(response.top[index].score) + '"><i class="material-icons inline">grade</i>' + response.top[index].score + '</div>' + '<div class="type">' + type + '</div>' + '<div class="run_date">' + start_date + '</div>' + '<div class="episodes">' + episodes + ' episodes</div>' + '<div class="image"><img src="' + response.top[index].image_url + '"></div>' + '</div>';
    });
    $('#list').html(sb);
  }
  ...
  $("#search_button").click(function () {
    ...
    if (search_value.length < 3) {
      ...
      $(response.results).each(function (index) {
        ...
        sb += '<div class="item">' + '<div class="rank">' + (index + 1) + '</div>' + '<div class="title">' + response.results[index].title + '</div>' + '<div class="score ' + getScoreColor(response.results[index].score) + '"><i class="material-icons inline">grade</i>' + response.results[index].score + '</div>' + '<div class="type">' + type + '</div>' + '<div class="run_date">' + start_date + ' - ' + end_date + '</div>' + '<div class="episodes">' + episodes + ' episodes</div>' + '<div class="image"><img src="' + response.results[index].image_url + '"></div>' + '</div>';
      });
      $('#list').html(sb);
    }
  });
