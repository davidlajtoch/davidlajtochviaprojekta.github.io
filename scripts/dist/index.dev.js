...
$(document).ready(function () {
  ...
  function search() {
    ...
    if (search_value.length < 3) {
      ...
      var search_string = $.trim(search_value);
  ...
  $('#list').on('click', '.anime_button', function () {
    ...
    var episodes = $(this).find('.episodes')[0].innerText;
    ...
    if (response.episodes.length === 0) {
      sb += '<div class="content_status highlight_faded">' + '<p class="face">' + sad_face[Math.floor(Math.random() * sad_face.length)] + '</p>' + '<p class="status_message">no episodes to display</p>' + '</div>';
    } else {
      $(response.episodes).each(function (index) {
        sb += '<div class="episode">' + '<div class="index">' + response.episodes[index].episode_id + '</div>' + '<div class="title">' + response.episodes[index].title + '</div>' + '<div class="title_japanese">' + response.episodes[index].title_japanese + '</div>' + '<div class="title_romanji">' + response.episodes[index].title_romanji + '</div>' + '<div class="aired">' + response.episodes[index].aired.substring(0, 10) + '</div>' + '</div>';
      });
    }

    sb += '</div>' + '</div>' + '<div class="section2">' + '<div class="lds-ellipsis" id="loading_animation"><div></div><div></div><div></div><div></div></div>' + '<div class="chart" id="chart_rating" style="position: relative; height: 320px; width: 50%;">' + '<canvas id="chartRating"></canvas>' + '</div>' + '<div class="chart" style="position: relative; height: 320px; width:50%;">' + '<canvas id="chartStats"></canvas>' + '</div></div>';
    setTimeout(function () {
      var config = new Config();
      var client = new Client(config);
      ...
      if (response.scores.length !== 0) {
        ...
        var chart_rating = new Chart(ctx1, {
          ...
          data: {
            ...
            datasets: [{
              ...
              backgroundColor: ['rgba(124, 77, 255, 0.3)', 'rgba(124, 77, 255, 0.3)', 'rgba(124, 77, 255, 0.3)', 'rgba(124, 77, 255, 0.3)', 'rgba(124, 77, 255, 0.3)', 'rgba(124, 77, 255, 0.3)', 'rgba(124, 77, 255, 0.3)', 'rgba(124, 77, 255, 0.3)', 'rgba(124, 77, 255, 0.3)', 'rgba(124, 77, 255, 0.3)'],
              borderColor: ['rgba(124, 77, 255,1)', 'rgba(124, 77, 255,1)', 'rgba(124, 77, 255,1)', 'rgba(124, 77, 255,1)', 'rgba(124, 77, 255,1)', 'rgba(124, 77, 255,1)', 'rgba(124, 77, 255,1)', 'rgba(124, 77, 255,1)', 'rgba(124, 77, 255,1)', 'rgba(124, 77, 255,1)'],
              borderWidth: 1
            }]
          },
          options: {
        ...
        var sb_no_rating = '<div class="content_status highlight_faded">' + '<p class="face">' + sad_face[Math.floor(Math.random() * sad_face.length)] + '</p>' + '<p class="status_message">no ratings to display</p>' + '</div>';
      ...
      var chart_stats = new Chart(ctx2, {
        ...
        data: {
          ...
          datasets: [{
            ...
            backgroundColor: ['rgba(124, 77, 255, 0.3)', 'rgba(124, 77, 255, 0.3)', 'rgba(124, 77, 255, 0.3)', 'rgba(124, 77, 255, 0.3)', 'rgba(124, 77, 255, 0.3)'],
            borderColor: ['rgba(124, 77, 255,1)', 'rgba(124, 77, 255,1)', 'rgba(124, 77, 255,1)', 'rgba(124, 77, 255,1)', 'rgba(124, 77, 255,1)'],
            borderWidth: 1
          }]
        },
        options: {
